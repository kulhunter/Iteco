...
    const showStep = (step) => {
        // Evitar error si el elemento no existe en la página
...
    };
    ...
    // Solo agregar listeners si el formulario de calculadora existe
    if (form) {
...
    let isTyping = false; // Estado para evitar spam de mensajes
    ...
    // Esta es la función que abre y cierra el chat
    const toggleChat = () => {
...
    };
    ...
    // 2. ASIGNARLA A 'window'
    // Esto es necesario para que el botón 'X' (onclick="toggleChat()") en el HTML funcione.
    window.toggleChat = toggleChat;

    // 3. CONECTAR EL BOTÓN FLOTANTE
    // Esto previene el error 'chatToggle is not defined' si el elemento no existe
    if (chatToggle) {
        chatToggle.addEventListener('click', toggleChat);
    }


    // --- EL RESTO DEL CÓDIGO DEL CHAT ---
...
    // Manejo del formulario de chat
    // Solo agregar listener si el formulario de chat existe
    if (chatForm) {
...
    D      }, 1200); // 1.2 segundos para simular la respuesta
        });
    }
});
